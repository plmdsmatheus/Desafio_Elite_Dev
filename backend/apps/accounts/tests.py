...
class TestRegister:
    ...
    def test_register_ignores_role_in_payload(self, api_client):
        """Nobody becomes an organizer/gate just by sending "role" in the public signup body."""
        response = api_client.post(
            "/api/auth/register",
...
class TestLoginAndMe:
    ...
    def test_me_with_real_bearer_token_from_login(self, api_client, customer):
        """Real end-to-end via the Authorization header, not force_authenticate — the
        same path the Swagger UI takes when you click "Authorize"."""
